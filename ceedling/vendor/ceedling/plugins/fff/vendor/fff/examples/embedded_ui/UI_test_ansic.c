...
#define RUN_TEST(SUITE, TESTNAME)                                              \
  printf(" Running %s.%s: \n", #SUITE, #TESTNAME);                             \
  setup();                                                                     \
  TESTNAME();                                                                  \
  printf(" SUCCESS\n");
...
TEST_F(
    UITests,
    when_no_empty_lines_write_line_clears_screen_and_outputs_lines_to_display) {
...
int main() {
  ...
  RUN_TEST(UITests,
           when_one_irq_and_valid_callback_then_missed_irq_counter_zero);
